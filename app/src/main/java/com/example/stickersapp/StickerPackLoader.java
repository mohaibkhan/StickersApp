...
import static com.example.stickersapp.StickerContentProvider.ANDROID_APP_DOWNLOAD_LINK_IN_QUERY;
import static com.example.stickersapp.StickerContentProvider.AVOID_CACHE;
import static com.example.stickersapp.StickerContentProvider.IMAGE_DATA_VERSION;
import static com.example.stickersapp.StickerContentProvider.IOS_APP_DOWNLOAD_LINK_IN_QUERY;
import static com.example.stickersapp.StickerContentProvider.LICENSE_AGREENMENT_WEBSITE;
import static com.example.stickersapp.StickerContentProvider.PRIVACY_POLICY_WEBSITE;
import static com.example.stickersapp.StickerContentProvider.PUBLISHER_EMAIL;
import static com.example.stickersapp.StickerContentProvider.PUBLISHER_WEBSITE;
import static com.example.stickersapp.StickerContentProvider.STICKER_FILE_EMOJI_IN_QUERY;
import static com.example.stickersapp.StickerContentProvider.STICKER_FILE_NAME_IN_QUERY;
import static com.example.stickersapp.StickerContentProvider.STICKER_PACK_ICON_IN_QUERY;
import static com.example.stickersapp.StickerContentProvider.STICKER_PACK_IDENTIFIER_IN_QUERY;
import static com.example.stickersapp.StickerContentProvider.STICKER_PACK_NAME_IN_QUERY;
import static com.example.stickersapp.StickerContentProvider.STICKER_PACK_PUBLISHER_IN_QUERY;
...
class StickerPackLoader {
    ...
    static ArrayList<StickerPack> fetchStickerPacks(Context context) throws IllegalStateException {
        ...
        if (cursor == null) {
            throw new IllegalStateException("could not fetch from content provider, " + "com.example.stickersapp.stickercontentprovider");
        }
    ...
    private static Uri getStickerListUri(String identifier) {
        return new Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority("com.example.stickersapp.stickercontentprovider").appendPath(StickerContentProvider.STICKERS).appendPath(identifier).build();
    }

    static Uri getStickerAssetUri(String identifier, String stickerName) {
        return new Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority("com.example.stickersapp.stickercontentprovider").appendPath(StickerContentProvider.STICKERS_ASSET).appendPath(identifier).appendPath(stickerName).build();
    }
}
